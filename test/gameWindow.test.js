...
test("isGameDayWindowNow is true when in-window (8-20 Helsinki) and an upcoming event starts today", () => {
...
test("isGameDayWindowNow is false when it's a game day but outside the hour window", () => {
  const now = new Date("2026-10-08T03:00:00.000Z"); // 06:00 Helsinki — before 8:00
...
test("isGameDayWindowNow boundary: exactly 7:00 Helsinki is outside the window (exclusive)", () => {
  const now = new Date("2026-10-08T04:00:00.000Z"); // 07:00 Helsinki exactly
  assert.equal(isGameDayWindowNow(UPCOMING_JYP_OCT8, now), false);
});

test("isGameDayWindowNow boundary: exactly 8:00 Helsinki is inside the window (inclusive)", () => {
  const now = new Date("2026-10-08T05:00:00.000Z"); // 08:00 Helsinki exactly
  assert.equal(isGameDayWindowNow(UPCOMING_JYP_OCT8, now), true);
});

test("isGameDayWindowNow boundary: exactly 19:00 Helsinki is inside the window (inclusive)", () => {
  const now = new Date("2026-10-08T16:00:00.000Z"); // 19:00 Helsinki exactly
  assert.equal(isGameDayWindowNow(UPCOMING_JYP_OCT8, now), true);
});

test("isGameDayWindowNow boundary: exactly 20:00 Helsinki is outside the window (exclusive)", () => {
  const now = new Date("2026-10-08T17:00:00.000Z"); // 20:00 Helsinki exactly
  assert.equal(isGameDayWindowNow(UPCOMING_JYP_OCT8, now), false);
});
...
test("isGameDayWindowNow near-midnight DST case: Helsinki date has already rolled to the next day", () => {
  ...
  // Hour is 01 (well outside 8-20), so this is false regardless of which date an
...
test("decide() returns the literal string 'skip' outside the window or on a non-game day", async () => {
  ...
  assert.equal(await decide(dataDirGameDay, new Date("2026-10-08T03:00:00.000Z")), "skip"); // 06:00 Helsinki — before 8:00
});
